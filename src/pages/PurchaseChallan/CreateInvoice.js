...
const CreateChallan = () => {
  ...
  const addInvoice = async () => {
    if (upperData.party == "") {
      ...
      if (resp.data.success == 1) {
        ...
        setTimeout(() => {
          history.push("/admin/v1/purchase-challan");
        }, 1500);
      } else {
        Toast.fire({
          icon: "error",
  ...
  const billNoGenerate = async (curDate) => {
    dispatch(setLoader(true));
    const data = await getChallanNo(user.token, {
      date: curDate,
      type: "Purchase",
    });
    if (data.data.no) {
  ...
  const fetchInvoiceData = async (invoiceId) => {
    dispatch(setLoader(true));
    const resp = await getChallanDetails(user.token, invoiceId);
    dispatch(setLoader(false));
          ...
          <CardBody>
            ...
            <Row className="justify-content-md-end mr-0">
              ...
              <Button
                ...
                onClick={() => history.push("/admin/v1/purchase-challan")}
              >
                Cancel
              </Button>
            </Row>
