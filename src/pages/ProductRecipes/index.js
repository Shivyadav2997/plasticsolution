...
const ProductRecipes = () => {
  ...
  const editClick = (cellData, rowData, row, col) => {
    ...
    history.push(`/admin/${user.path}/recipe-create?recipe=${id}`);
  };
  ...
  const getData = async () => {
    ...
    var data = {};
    // if (selMonth > 0) {
    data = await recipeListGet(user.token, "", "", selMonth);
    // } else {
    //   data = await recipeListGet(
    //     user.token,
    //     filterDate.st,
    //     filterDate.et,
    //     "",
    ...
    if (selMonth > 0) {
      ...
      if (data.data) {
        var data2 = data.data;
        setRecipes({
          all: data2.productracipes_list || [],
          monthly: data2.monthly_purchase || [],
        });
      } else {
        setRecipes({ all: [], monthly: [] });
      }
    }
    setLoading(false);
  };
              ...
              <Col>
                <Row className="justify-content-end mr-0">
                  <Button
                    ...
                    onClick={() =>
                      history.push(`/admin/${user.path}/recipe-create`)
                    }
                  >
                    Create Recipe
                  </Button>
