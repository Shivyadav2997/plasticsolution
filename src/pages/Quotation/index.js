...
const Quotation = () => {
  ...
  const [monthQuotations, setmonthQuotations] = useState([]);
  ...
  const editClick = (cellData, rowData, row, col) => {
    ...
    history.push(`/admin/v1/quotation-create?invoice=${id}`);
  };
  ...
  const viewInvoice = async (rowData) => {
    ...
    handleToggle(true);
    dispatch(setLoader(true));
    const resp = await quotationGet(user.token, {
      id: id,
      a: 1,
      h: 0,
      d: 0,
            ...
            <div className="d-flex gap-10">
              ...
              <div>
                <Button
                  ...
                  color="default"
                  onClick={() => {
                    sessionStorage.setItem("quotationIds", cellData.id);
                    history.push("/admin/v1/sales-invoice");
                  }}
                >
                  Invoice
                </Button>
  ...
  const getData = async () => {
    ...
    var data = {};
    if (selMonth > 0) {
      data = await quotationListGet(user.token, "", "", selMonth);
    } else {
      data = await quotationListGet(
        user.token,
        filterDate.st,
        filterDate.et,
        "",
    ...
    if (selMonth > 0) {
      if (data.data) {
        var data2 = data.data;
        setmonthQuotations(data2.quotation || []);
      } else {
        setmonthQuotations([]);
      }
    } else {
      if (data.data) {
        ...
        setQuotations({
          all: data2.quotation || [],
          monthly: data2.monthly_quotatation || [],
        });
      } else {
        setQuotations({ all: [], monthly: [] });
      }
    }
    setLoading(false);
  };
  ...
  const downloadOrWhatsappInvoice = async (whatsapp, mob) => {
    dispatch(setLoader(true));
    const resp = await quotationDownload(user.token, {
      id: invId,
      a: original ? 1 : 0,
      h: half ? 1 : 0,
      d: duplicate ? 1 : 0,
          ...
          <>
            <Row sm="2" className="mb-2">
              <Col className="">
                <Row className="ml-0">
                  <h1>
                    {selMonth}-{getMonthName(selMonth)} Quotations
                  </h1>
                  <Button
              ...
              <Col>
                <Row className="justify-content-end mr-0">
                  <Button
                    ...
                    onClick={() => history.push("/admin/v1/quotation-create")}
                  >
                    Create Quotation
                  </Button>
                </Row>
                ...
                <Row className="ml-0">
                  ...
                  <h1>
                    <span style={{ fontSize: "18px" }}>
                      {selParty.id != null && `${selParty.name} Quotations`}
                      {filterDate.st != "" &&
            ...
            <Row>
              <Col>
                {loading ? (
                  ...
                  <>
                    <CustomTab
                      tabnames={["All Quotation", "Monthly Quotation"]}
                      tabpanes={tabPan}
